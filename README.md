# Bitsave Stylus Contract
Bitsave protocol is a decentralized savings platform which
enables users to save in crypto. The crypto space has the 
feature of volatility which can be advantage or not. Bitsave
provides the security of finances in the crypto space while
giving the users the ability to decide if they want to roll
with the tide. 

The user flow can be found [here](flow.md).
...
### Comparison
Safe mode ensures that the value of amount saved is received at the end.  
Hence, drop in price due to volatility of token is avoided.  
The con of this 
is that profit in price of token is lost. Risk mode does the opposite of this.
